...
function PriceChart({ candles }) {
  ...
  const points = useMemo(() => {
    ...
    return data.map((item, index) => ({
      x: 8 + index * (284 / Math.max(data.length - 1, 1)),
      y: 110 - ((Number(item.close) - low) / range) * 92,
    }));
  }, [data]);
  if (!points.length) return <div className="grid h-full place-items-center text-sm text-slate-400">Historical candles will appear here.</div>;
  const line = points.map((point) => `${point.x},${point.y}`).join(" ");
  return <svg className="h-full w-full" viewBox="0 0 300 126" preserveAspectRatio="none" role="img" aria-label="Historical closing-price chart">
    {[20, 52, 84, 116].map((y) => <line key={y} x1="0" y1={y} x2="300" y2={y} stroke="currentColor" strokeOpacity="0.08" />)}
    <polyline points={line} fill="none" stroke="#2563eb" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
    <circle cx={points.at(-1).x} cy={points.at(-1).y} r="4" fill="#2563eb" stroke="white" strokeWidth="2" />
  </svg>;
}
  ...
  return <div className="h-screen overflow-hidden bg-slate-50 dark:bg-slate-950">
    <main className="mx-auto flex h-full max-w-[1600px] flex-col px-5 py-5 lg:px-8">
      ...
      <section className="mt-4 grid min-h-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-[210px_minmax(0,1fr)_300px]">
        ...
        <section className="panel flex min-h-0 flex-col p-5"><div className="flex items-start justify-between"><div><p className="metric-label">Market overview</p><h2 className="mt-1 text-2xl font-semibold tracking-tight">{selectedSymbol || "Select a symbol"}</h2></div><div className="rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs font-medium text-slate-500 dark:bg-slate-800 dark:text-slate-300">{configuration?.strategy.interval || "—"}</div></div><div className="mt-4 min-h-0 flex-1"><PriceChart candles={candles} /></div><div className="mt-2 flex items-center justify-between border-t border-slate-100 pt-3 text-xs text-slate-400 dark:border-slate-800"><span>Last 7 days</span><span>{candles.length ? `${candles.length} candles` : "No saved candles"}</span></div></section>
